# Perfiles — Arquitectura del microservicio

## Componentes

| Componente | Rol |
|---|---|
| `PerfilApplication` | Punto de entrada Spring Boot |
| `controller/PerfilController.java` | API REST expuesta en `/api/perfiles` |
| `service/PerfilService.java` | Lógica de negocio y transacciones |
| `repository/PerfilRepository.java` | Acceso JPA a H2 |
| `model/Perfil.java` | Entidad persistida en tablas H2 |
...
`Cliente/Sistema → API Gateway → PerfilController → PerfilService → PerfilRepository → H2`
