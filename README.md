# Perfiles — Microservicio de riesgo perfiles
...
administra los datos y la lógica del dominio de Perfiles del caso caso02 (TalentoYa). Su base de datos es una **H2 en memoria** (un solo microservicio por base), cumpliendo aislamiento de datos por dominio.
...
|--------|------|-------------|
| GET | `/api/perfiles` | Lista todos los recursos |
| GET | `/api/perfiles/{id}` | Obtiene un recurso por id |
| POST | `/api/perfiles` | Crea un recurso |
| PUT | `/api/perfiles/{id}` | Actualiza un recurso |
| DELETE | `/api/perfiles/{id}` | Elimina un recurso |
