...
class PerfilControllerTest {
    ...
    private Perfil recurso(Long id) {
        Perfil r = new Perfil();
        r.setId(id);
        r.setNombre("Demo");
        r.setEmail("valor");
    ...
    void obtenerInexistenteDevuelve404() throws Exception {
        ...
        mockMvc.perform(get("/api/perfiles/1")).andExpect(status().isNotFound());
    }
    ...
    void eliminarExistenteDevuelve204() throws Exception {
        ...
        mockMvc.perform(delete("/api/perfiles/1")).andExpect(status().isNoContent());
    }

    @Test
    void eliminarInexistenteDevuelve404() throws Exception {
        when(service.delete(1L)).thenReturn(false);
        mockMvc.perform(delete("/api/perfiles/1")).andExpect(status().isNotFound());
    }
}
